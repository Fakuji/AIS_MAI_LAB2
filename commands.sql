...
explain select * from Author where first_name='Nazar';
explain select * from Author where title='Ibragim';
explain select * from Author where id>10 and id<20;
explain select * from Author where first_name='Alex%';
insert into Author (first_name,last_name,email,title) values ('Александр','Александрович','alex@yandex.ru','студент');
...
explain select * from Author where email='giga_man@mail.ru';
...
explain format=json select * from Author where first_name='Alex%' and last_name='A%';